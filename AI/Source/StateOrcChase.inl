Grid<float>* StateOrcChase::im_Grid = nullptr;
Publisher* StateOrcChase::im_Publisher = Publisher::RetrieveGlobalObjPtr();

void StateOrcChase::Enter(Entity* const entity){
	...
	if(im_Publisher->Notify((long int)ListenerFlags::ObjPool, new EventFindClosestEnemy(entity), false)){
		...
		if((entityGridTargetLocalPos - entityLocalPos).Length() < entity->GetSpd() * (float)dt){
			...
			if(vecLenSquared - 1.0f <= Math::EPSILON && 1.0f - vecLenSquared <= Math::EPSILON){
				entity->SetNextState(entity->GetStateMachine()->GetState(StateID::StateOrcAttack));
				return;
			}
...
void StateOrcChase::Exit(Entity* const entity){
}